...
import { createDelivery } from '@/lib/api/delivery';
...
export default function CreateDeliveryPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        delivery_address: '',
        customer_phone: '',
        scheduled_date: getCurrentGregorianDate(),
        scheduled_time: '',
        status: 'pending',
        priority: 1,
        distance: '',
    ...
    const fetchRelatedData = async () => {
        try {
            const [customersRes, salesOrdersRes, driversRes] = await Promise.all([
                getContacts(),
                getSalesOrders(),
                getDrivers()
            ]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('حواله با موفقیت ایجاد شد');
            router.push('/dashboard/delivery/deliveries');
        } catch (error) {
            console.error('Error creating delivery:', error);
            alert('خطا در ایجاد حواله');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <PageHeader title="حواله جدید" subtitle="ایجاد حواله حمل جدید" />
            <div className="bg-white rounded-lg shadow p-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">تلفن مشتری</label>
                            <input type="tel" name="customer_phone" value={formData.customer_phone} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="number" name="priority" value={formData.priority} onChange={handleChange} min="1" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">مسافت (کیلومتر)</label>
                            <input type="number" step="0.01" name="distance" value={formData.distance} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="tracking_code" value={formData.tracking_code} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                    </div>
                    <div>
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        <button type="submit" disabled={loading} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-400">{loading ? 'در حال ذخیره...' : 'ذخیره'}</button>
                    </div>
                </form>
            </div>
        </div>
