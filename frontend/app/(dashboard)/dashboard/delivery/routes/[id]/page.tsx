...
import { getDeliveryRoute, updateDeliveryRoute } from '@/lib/api/delivery';
...
export default function EditRoutePage() {
    ...
    const [formData, setFormData] = useState({
        route_name: '', driver: '', date: '', status: 'planned', start_location: '', end_location: '',
        total_distance: '0', estimated_duration: '0', notes: '', company: '1'
    });
    ...
    const fetchData = async () => {
        try {
            const [routeRes, driversRes] = await Promise.all([
                getDeliveryRoute(params.id as string),
                getDrivers()
            ]);
            setFormData(routeRes.data);
            setDrivers(driversRes.data.filter((d: any) => d.is_active));
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await updateDeliveryRoute(params.id as string, formData);
            alert('مسیر با موفقیت بروزرسانی شد');
            router.push('/dashboard/delivery/routes');
        } catch (error) {
            console.error('Error updating route:', error);
            alert('خطا در بروزرسانی مسیر');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div>
            <PageHeader title="ویرایش مسیر" subtitle="ویرایش اطلاعات مسیر حمل" />
            <div className="bg-white rounded-lg shadow p-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        <button type="submit" disabled={saving} className="bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700 transition-colors disabled:bg-gray-400">{saving ? 'در حال ذخیره...' : 'ذخیره'}</button>
                    </div>
                </form>
            </div>
        </div>
