...
import { createSalesOrder, createSalesOrderItem, getCustomers, getItems } from '@/lib/api/sales';
...
export default function CreateSalesOrderPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccess('سفارش فروش با موفقیت ایجاد شد');
            setTimeout(() => {
                router.push('/dashboard/sales/sales-orders');
            }, 1500);
        } catch (err: any) {
            console.error(err);
            setError(err.response?.data?.detail || 'خطا در ایجاد سفارش');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <PageHeader
                title="افزودن سفارش فروش جدید"
                subtitle="ایجاد سفارش فروش جدید در سیستم"
            />
