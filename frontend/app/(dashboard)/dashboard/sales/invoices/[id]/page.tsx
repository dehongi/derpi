...
export default function EditInvoicePage() {
    ...
    const fetchData = async () => {
        try {
            const [invoiceRes, customersRes, itemsRes, ordersRes] = await Promise.all([
            ...
            const itemsResponse = await api.get(`/sales/invoice-items/`);
            const filteredItems = itemsResponse.data.filter((item: any) => item.invoice === parseInt(id as string));
            setInvoiceItems(filteredItems.length > 0 ? filteredItems : [{ item: '', quantity: 1, unit_price: 0, discount: 0, total: 0 }]);
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await updateInvoice(id as string, invoiceData);

            const existingItems = await api.get(`/sales/invoice-items/`);
            const itemsToDelete = existingItems.data.filter((item: any) => item.invoice === parseInt(id as string));

            for (const item of itemsToDelete) {
                await api.delete(`/sales/invoice-items/${item.id}/`);
            }

            for (const item of invoiceItems) {
                if (!item.item) continue;
                await api.post('/sales/invoice-items/', {
                    invoice: id,
                    item: item.item,
    ...
    return (
        <div>
            <PageHeader
                title="ویرایش فاکتور"
                subtitle={`ویرایش فاکتور ${formData.invoice_number}`}
            />
