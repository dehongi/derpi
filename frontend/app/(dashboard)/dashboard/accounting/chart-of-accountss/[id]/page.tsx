...
export default function EditChartOfAccountsPage() {
    ...
    const fetchItem = async () => {
        try {
            const response = await api.get(`/accounting/chart-of-accountss/${id}/`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.put(`/accounting/chart-of-accountss/${id}/`, formData);
            setSuccess('تغییرات با موفقیت ذخیره شد');
            setTimeout(() => {
                router.push('/dashboard/accounting/chart-of-accountss');
            }, 1500);
        } catch (err: any) {
    ...
    const handleDelete = async () => {
        if (confirm('آیا از حذف این مورد اطمینان دارید؟')) {
            try {
                await api.delete(`/accounting/chart-of-accountss/${id}/`);
                router.push('/dashboard/accounting/chart-of-accountss');
            } catch (error) {
        ...
        <div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded shadow p-6">
                ...
                <div className="mt-6 flex gap-4">
                    ...
                    <button
                        type="button"
                        onClick={() => router.push('/dashboard/accounting/chart-of-accountss')}
