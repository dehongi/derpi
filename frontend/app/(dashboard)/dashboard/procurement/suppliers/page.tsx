...
export default function SuppliersPage() {
    ...
    const handleEdit = (item: any) => {
        router.push(`/dashboard/procurement/suppliers/${item.id}`);
    };

    const handleDelete = async (item: any) => {
        if (confirm('آیا از حذف این مورد اطمینان دارید؟')) {
            try {
                await api.delete(`/procurement/suppliers/${item.id}/`);
                fetchItems();
            } catch (error) {
    ...
    return (
        <div>
            <PageHeader
                title="تامین‌کنندگان"
                subtitle="مدیریت تامین‌کنندگان"
                action={
                    <button
                        onClick={() => router.push('/dashboard/procurement/suppliers/create')}
